...
|------|----------|----------|--------|------|--------|----------|------|
| `min-score 60` | 8511 | 107 | 33.89% | 98.85% | 9.00 | -1.14% | 42.06% |
| `min-score 70` | 2769 | 106 | 38.08% | 115.33% | 9.04 | -0.86% | 41.51% |
| `min-score 75` | 500 | 100 | 28.87% | 89.47% | 4.56 | -2.86% | 16.00% |

这组结果只能作为工程验证和参数扫描起点。当前回测已加入“执行日涨停开盘不买入”约束，`min-score 70` 样本中有 39 次买入被跳过；卖出端跌停约束、开盘滑点异常处理和真实盘口容量约束仍待增强。
